...
def attach_stores(n, costs, config):
    ...
    # Check if any H2 storage is configured
    # "H2" in config triggers BOTH tank and cavern creation (smart default)
    h2_carriers = []
    if "H2" in carriers:
        h2_carriers = ["H2_tank", "H2_cavern"]
        logger.info("'H2' detected in config - will create both H2_tank and H2_cavern stores")
    else:
        h2_carriers = [c for c in carriers if c in ["H2_tank", "H2_cavern"]]
    
    if h2_carriers:
        # Create H2 buses ONCE (shared by all H2 storage types)
        h2_buses_i = n.madd("Bus", buses_i + " H2", carrier="H2", **bus_sub_dict)
        
        # Define geologically feasible regions for cavern storage
        cavern_feasible_buses = ["AR.COM", "AR.PAT"]
        
        # Add H2_tank stores (available everywhere)
        if "H2_tank" in h2_carriers:
            n.madd(
                "Store",
                h2_buses_i + " H2_tank",
                bus=h2_buses_i,
            ...
            logger.info(f"Added {len(h2_buses_i)} H2_tank stores at all buses")
        
        # Add H2_cavern stores (restricted to feasible regions)
        if "H2_cavern" in h2_carriers:
            # Create cavern stores at all buses
            cavern_stores = n.madd(
                "Store",
                h2_buses_i + " H2_cavern",
                bus=h2_buses_i,
                carrier="H2_cavern",
                e_nom_extendable=True,
                e_cyclic=True,
            ...
            # Restrict caverns to geologically feasible regions
            # Set e_nom_max = 0 for non-feasible regions
            infeasible_mask = ~buses_i.isin(cavern_feasible_buses)
            infeasible_stores = h2_buses_i[infeasible_mask] + " H2_cavern"
            
            n.stores.loc[infeasible_stores, "e_nom_max"] = 0.0
            n.stores.loc[infeasible_stores, "p_nom_max"] = 0.0
            
            logger.info(
                f"Added {len(h2_buses_i)} H2_cavern stores - "
                f"allowed in {len(cavern_feasible_buses)} regions: {', '.join(cavern_feasible_buses)}, "
                f"restricted in {infeasible_mask.sum()} regions"
            )
        
        # Add conversion links ONCE (shared by all H2 storage types)
        n.madd(
            "Link",
